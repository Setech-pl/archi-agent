...
export const sequencePlannerInstructions = [
  ...
  "- A message between two different known participants must use one supplied relationship with the same fromId and toId (for a response, the reverse of its request), the same interfaceType and the same mode.",
  "- Every message must include both async and isResponse as true or false. Never omit either field.",
  "- Set async from the mode of the relationship used: asynchronous gives async true, synchronous gives async false.",
  ...
  "- Set isResponse to false for a request or event. Set isResponse to true only for a synchronous reply to an earlier synchronous request between the same two participants in the opposite direction. A response always has async false.",
