...
installed Node.js. This document describes the foundation checkpoint; it is not published to any
marketplace.
...
| Core and Node adapters | `src/core`, `src/node` | as before | Unchanged. |
...
adapter) and a result carrying the validated PlantUML, the grounding report and safe issues. Later
...
3. When no model is configured, the models reported by the local server are listed; the choice is
...
| `archiAgent.localModel.baseUrl` | `http://127.0.0.1:1234/v1` | Loopback base URL of the local OpenAI-compatible server. Only `http://127.0.0.1:<port>/v1` and `http://[::1]:<port>/v1` are accepted (see `docs/local-model.md`). |
| `archiAgent.localModel.model` | (empty) | Model identifier reported by the server. When empty, the command lists the models and stores the choice here. |
...
1. Start LM Studio (or another OpenAI-compatible server) on the loopback interface, by default
   port 1234, and load an instruction model that supports structured JSON output. Very small models
   often fail the structured-output contract or the grounding rules.
2. Leave `archiAgent.localModel.baseUrl` at the default or set the loopback port you use.
3. Either set `archiAgent.localModel.model` to the identifier the server reports, or leave it empty
   and pick one when the command asks.
...
- Only one Knowledge Pack directory and one local OpenAI-compatible endpoint can be configured.
